...
#   PALADIN_BIN=target/release/paladin ruby tests/e2e/regenerate_goldens.rb
#
...
unless File.executable?(Golden.binary)
  warn "paladin binary not found at #{Golden.binary}"
...
def run!(args, stdin: nil)
  ...
  warn "golden generation failed (exit #{status.exitstatus}): paladin #{args.join(' ')}"
  warn err
  warn out
  exit 1
end
...
Golden.manifest.each do |g|
  # -f lets paladin overwrite an existing golden; the script-level --force guard
