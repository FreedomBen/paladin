...
class MiscTest < E2ETest
  ...
  def test_header_is_authenticated_no_downgrade
    cipher = good_cipher("dg.symcrypt") # encrypted with the default aes-256-gcm
  ...
  def test_reserved_flag_bit_is_rejected
    cipher = good_cipher("rf.symcrypt")
    set_byte(cipher, 11, 0x04) # flags byte: set a reserved bit

    assert_failure symcrypt("-i", cipher), 4, "reserved flags"
    res = symcrypt("-d", cipher, "-o", tmp("o"), "--password-env", "PW",
                   env: { "PW" => DEFAULT_PASSWORD })
  ...
  def test_fresh_randomness_per_file
    ...
    c1 = tmp("r1.symcrypt")
    c2 = tmp("r2.symcrypt")
    symcrypt("-e", Symcrypt::LOREM, "-o", c1, *FAST_KDF, "--password-env", "PW", env: env)
    symcrypt("-e", Symcrypt::LOREM, "-o", c2, *FAST_KDF, "--password-env", "PW", env: env)
    ...
    o1 = tmp("r1.out")
    o2 = tmp("r2.out")
    assert_status symcrypt("-d", c1, "-o", o1, "--password-env", "PW", env: env), 0
    assert_status symcrypt("-d", c2, "-o", o2, "--password-env", "PW", env: env), 0
    assert_identical Symcrypt::LOREM, o1
    assert_identical Symcrypt::LOREM, o2
  end

  # `-q` produces no stderr chatter on a successful file operation.
  def test_quiet_is_silent_on_success
    env    = { "PW" => DEFAULT_PASSWORD }
    cipher = tmp("q.symcrypt")
    enc = symcrypt("-e", Symcrypt::LOREM, "-o", cipher, "-q", *FAST_KDF,
                   "--password-env", "PW", env: env)
    assert_status enc, 0
    assert_empty enc.stderr, "--quiet encrypt should not chatter on stderr"

    dec = symcrypt("-d", cipher, "-o", tmp("q.out"), "-q",
                   "--password-env", "PW", env: env)
    assert_status dec, 0
  ...
  def test_help_and_version
    ...
    version = symcrypt("--version")
    assert_status version, 0
    assert_match(/symcrypt \d+\.\d+\.\d+/, version.stdout)
  end
