...
class KeyfileTest < E2ETest
  ...
  def test_password_plus_keyfile_round_trip
    kf     = make_keyfile
    cipher = tmp("pk.paladin")
    out    = tmp("pk.out")
    env    = { "PW" => DEFAULT_PASSWORD }
    enc = paladin("-e", Paladin::LOREM, "-o", cipher, "-k", kf, *FAST_KDF,
                   "--password-env", "PW", env: env)
    assert_status enc, 0
    dec = paladin("-d", cipher, "-o", out, "-k", kf, "--password-env", "PW", env: env)
    assert_status dec, 0
  ...
  def test_keyfile_only_round_trip
    kf     = make_keyfile
    cipher = tmp("ko.paladin")
    out    = tmp("ko.out")
    enc = paladin("-e", Paladin::LOREM, "-o", cipher, "-k", kf, "--no-password", *FAST_KDF)
    assert_status enc, 0
    dec = paladin("-d", cipher, "-o", out, "-k", kf, "--no-password")
    assert_status dec, 0
    assert_identical Paladin::LOREM, out
  end
  ...
  def test_wrong_keyfile_is_auth_failure
    ...
    cipher = tmp("wk.paladin")
    paladin("-e", Paladin::LOREM, "-o", cipher, "-k", good, "--no-password", *FAST_KDF)
    res = paladin("-d", cipher, "-o", tmp("o"), "-k", bad, "--no-password")
  ...
  def test_missing_keyfile_on_decrypt_is_auth_failure
    kf     = make_keyfile
    cipher = tmp("mk.paladin")
    env    = { "PW" => DEFAULT_PASSWORD }
    paladin("-e", Paladin::LOREM, "-o", cipher, "-k", kf, *FAST_KDF,
             "--password-env", "PW", env: env)

    without = paladin("-d", cipher, "-o", tmp("o1"), "--password-env", "PW", env: env)
    assert_failure without, 3, "authentication failed"

    with = paladin("-d", cipher, "-o", tmp("o2"), "-k", kf, "--password-env", "PW", env: env)
  ...
  def test_two_keyfiles_yield_different_keys
    ...
    c1  = tmp("c1.paladin")
    c2  = tmp("c2.paladin")
    paladin("-e", Paladin::LOREM, "-o", c1, "-k", kf1, "--no-password", *FAST_KDF)
    paladin("-e", Paladin::LOREM, "-o", c2, "-k", kf2, "--no-password", *FAST_KDF)

    # Each keyfile decrypts its own file...
    assert_status paladin("-d", c1, "-o", tmp("o1"), "-k", kf1, "--no-password"), 0
    assert_status paladin("-d", c2, "-o", tmp("o2"), "-k", kf2, "--no-password"), 0
    # ...but not the other's.
    assert_failure paladin("-d", c1, "-o", tmp("x1"), "-k", kf2, "--no-password"), 3
    assert_failure paladin("-d", c2, "-o", tmp("x2"), "-k", kf1, "--no-password"), 3
  end

  # `-k -` is rejected: stdin is reserved for the main input stream (exit 2).
  def test_keyfile_from_stdin_is_rejected
    res = paladin("-e", Paladin::LOREM, "-o", tmp("o"), "-k", "-", "--no-password", *FAST_KDF)
  ...
  def test_missing_keyfile_path_is_usage_error
    res = paladin("-e", Paladin::LOREM, "-o", tmp("o"), "-k", tmp("nope.bin"),
