...
class PasswordSourcesTest < E2ETest
  ...
  def test_inline_password_round_trip
    cipher = tmp("inline.paladin")
    out    = tmp("inline.out")
    enc = paladin("-e", Paladin::LOREM, "-o", cipher, *FAST_KDF, "-p", DEFAULT_PASSWORD)
    assert_status enc, 0
    dec = paladin("-d", cipher, "-o", out, "-p", DEFAULT_PASSWORD)
    assert_status dec, 0
  ...
  def test_password_file_round_trip
    ...
    cipher  = tmp("file.paladin")
    out     = tmp("file.out")
    enc = paladin("-e", Paladin::LOREM, "-o", cipher, *FAST_KDF, "--password-file", pw_file)
    assert_status enc, 0
    dec = paladin("-d", cipher, "-o", out, "--password-file", pw_file)
    assert_status dec, 0
    assert_identical Paladin::LOREM, out
  end
  ...
  def test_sources_interoperate
    cipher = tmp("interop.paladin")
    out_p  = tmp("interop_p.out")
    out_f  = tmp("interop_f.out")
    pw_file = write_input("pw", DEFAULT_PASSWORD) # no trailing newline

    enc = paladin("-e", Paladin::LOREM, "-o", cipher, *FAST_KDF,
                   "--password-env", "PW", env: { "PW" => DEFAULT_PASSWORD })
    assert_status enc, 0

    dec_p = paladin("-d", cipher, "-o", out_p, "-p", DEFAULT_PASSWORD)
    assert_status dec_p, 0
    assert_identical Paladin::LOREM, out_p

    dec_f = paladin("-d", cipher, "-o", out_f, "--password-file", pw_file)
    assert_status dec_f, 0
    assert_identical Paladin::LOREM, out_f
  end
  ...
  def test_password_mismatch_is_auth_failure
    cipher = tmp("mismatch.paladin")
    paladin("-e", Paladin::LOREM, "-o", cipher, *FAST_KDF, "-p", "alpha")
    res = paladin("-d", cipher, "-o", tmp("o"), "-p", "beta")
  ...
  def test_multiple_sources_is_usage_error
    pw_file = write_input("pw", DEFAULT_PASSWORD)
    res = paladin("-e", Paladin::LOREM, "-o", tmp("o"), *FAST_KDF,
  ...
  def test_empty_password_file_is_usage_error
    ...
    res = paladin("-e", Paladin::LOREM, "-o", tmp("o"), *FAST_KDF, "--password-file", empty)
