//! Format a `paladin_core::Header` into display rows for the Info pane,
...
pub fn metadata_rows(meta: &Metadata) -> Vec<InfoRow> {
    match meta {
        Metadata::Paladin(header) => header_rows(header),
...
/// Render a paladin `header` as `--info` text. Byte-identical to the CLI.
...
mod tests {
    ...
    fn full_rows_with_present_name() {
        ...
        let expected_text = "format: paladin\n\
             version: 1\n\
             cipher: aes-256-gcm\n\
             kdf: argon2id\n\
    ...
    fn aescrypt_rows_and_text_match_the_cli_block() {
        use paladin_core::{AesCryptHeader, AesCryptKdf, Metadata};
