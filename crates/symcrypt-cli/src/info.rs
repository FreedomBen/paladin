...
//! integration tests assert it byte-for-byte for both the symcrypt and the AES
//! Crypt block.
...
pub fn format_info(meta: &Metadata) -> String {
    match meta {
        Metadata::Symcrypt(header) => format_symcrypt(header),
        Metadata::AesCrypt(header) => format_aescrypt(header),
    }
}

/// The native symcrypt 12-line block.
fn format_symcrypt(header: &Header) -> String {
