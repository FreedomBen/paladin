//! `paladin-common` — terminal glue shared by the CLI and TUI front-ends:
...
//! (DESIGN §2.2, §6.4–§6.6). It depends only on `paladin-core` and std (plus
//! `thiserror`, `tempfile`, and `zeroize`); `paladin-gtk` deliberately does not
//! use it.
