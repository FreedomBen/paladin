//! Integration tests for the `paladin` CLI binary.
//!
...
/// Build a fresh invocation of the `paladin` binary under test.
fn sc() -> Command {
...
/// 2. Encrypt with no `-o` appends `.paladin` beside the input.
#[test]
fn default_encrypt_extension_is_paladin() {
    let tmp = TempDir::new().unwrap();
    let dir = tmp.path();
    ...
    assert!(dir.join("report.pdf.paladin").exists());
}

/// 3. Encrypt with `-a` appends `.paladin.asc` beside the input.
#[test]
fn default_armored_extension_is_paladin_asc() {
    let tmp = TempDir::new().unwrap();
    let dir = tmp.path();
    fs::write(p(dir, "report.pdf"), b"pdf-bytes").unwrap();
    ...
    assert!(dir.join("report.pdf.paladin.asc").exists());
}
...
/// 7. Decrypt strips a recognized `.paladin` extension when no `-o` given.
#[test]
fn decrypt_strips_known_extension() {
    ...
    sc().args([
        ...
        "10000",
        "-o",
        &p(dir, "s/payload.paladin"),
    ])
    .assert()
    .success();

    sc().args(["-d", &p(dir, "s/payload.paladin"), "-p", "pw"])
        .assert()
        .success();
...
/// 9. Decrypt of a file named exactly `.paladin` falls back to `.paladin.dec`.
#[test]
fn decrypt_empty_basename_fallback() {
    ...
    sc().args(["-d", &p(dir, "s/.paladin"), "-p", "pw"])
        .assert()
        .success();

    let restored = dir.join("s").join(".paladin.dec");
    assert!(restored.exists());
    assert_eq!(fs::read(restored).unwrap(), plain);
}
