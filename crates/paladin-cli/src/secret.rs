...
//! `--no-password`) are resolved by `paladin-common`; when none is supplied we
...
//! at-least-one-byte rules are enforced upstream (`paladin-common` /
...
fn prompt_password(confirm: bool) -> AppResult<Zeroizing<Vec<u8>>> {
    loop {
        ...
        if first.is_empty() {
            eprintln!("paladin: empty password; use --no-password with -k for keyfile-only mode");
            continue;
        }
        if confirm {
            let second =
            ...
            if first.as_slice() != second.as_slice() {
                eprintln!("paladin: passwords did not match; try again");
                continue;
            }
        }
        return Ok(first);
