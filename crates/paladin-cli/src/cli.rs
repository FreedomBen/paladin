...
/// `paladin` command-line arguments.
#[derive(Parser, Debug)]
#[command(
    name = "paladin",
    version,
    ...
    long_about = "Simple, safe symmetric file encryption.\n\n\
        ...
        writes paladin's own format, never AES Crypt; to migrate a .aes file, \
        decrypt it and re-encrypt with paladin. An AES Crypt key file may be \
        supplied via --password-file only when it is valid UTF-8 text (paladin \
...
pub struct Cli {
    ...
    #[arg(short = 'e', long)]
    pub encrypt: bool,
    /// Decrypt FILE to its plaintext (paladin or AES Crypt `.aes`, auto-detected).
    #[arg(short = 'd', long)]
    pub decrypt: bool,
    /// Print unauthenticated header metadata; no password needed (paladin or AES Crypt).
    #[arg(short = 'i', long)]
    pub info: bool,
    /// Decrypt-and-discard to verify integrity and password (paladin or AES Crypt).
    #[arg(long)]
    pub verify: bool,
    ...
    /// to paladin files only — not to AES Crypt `.aes` files.
