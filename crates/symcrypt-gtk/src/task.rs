...
use symcrypt_core::{decrypt, encrypt, verify, EncryptOptions, Progress, Secret, SymError};
...
mod tests {
    ...
    use std::io::Write;
    use std::path::Path;
    use symcrypt_core::{CipherId, KdfId, KdfParams};
    ...
    fn make_ciphertext(plaintext: &[u8]) -> (tempfile::TempDir, PathBuf, PathBuf) {
        ...
        let output = dir.path().join("plain.symcrypt");
    ...
    fn round_trip_encrypt_then_decrypt() {
        ...
        let ciphertext = dir.path().join("plain.txt.symcrypt");
    ...
    fn overwrite_approved_replaces_file() {
        ...
        let res = run_job(job, &cancel, noop);
        assert!(res.is_ok());
        // It is now a valid symcrypt container, not the old bytes.
