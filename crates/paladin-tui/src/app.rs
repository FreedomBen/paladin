//! Application state for the paladin TUI. `event` mutates it; `ui` renders it.

use std::path::{Path, PathBuf};

use paladin_common as common;
...
impl App {
    ...
    /// exit code is mapped through `paladin-common`. A cancellation is a
    ...
    /// stored name (paladin) or strips `.aes` (AES Crypt) and hints when the
                    ...
                    Ok(mut reader) => match core::inspect(&mut reader) {
                        ...
                        Err(_) => {
                            self.field_error = Some(
                                "not a recognized paladin or AES Crypt file; enter the output path"
                                    .to_string(),
                            );
                        }
                    },
...
mod tests {
    ...
    fn rejects_missing_file() {
        assert!(validate_input("/no/such/paladin/path/xyz").is_err());
    }
    ...
    fn encrypt_prefill_follows_armor_extension() {
        ...
        assert!(app.output.text().ends_with(".paladin"));
        app.toggle(Focus::Armor);
        assert!(app.output.text().ends_with(".paladin.asc"));
    }
    ...
    #[test]
    fn info_on_non_paladin_file_maps_to_format_error() {
        let dir = tempfile::tempdir().unwrap();
        let junk = dir.path().join("junk.bin");
        std::fs::write(&junk, b"not a paladin file at all").unwrap();
