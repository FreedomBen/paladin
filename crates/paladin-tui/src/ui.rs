//! ratatui rendering for the paladin TUI. Kept thin: it reads [`App`] state and
...
fn draw_tabs(f: &mut Frame, app: &App, area: Rect) {
    ...
    let tabs = Tabs::new(titles)
        .block(Block::bordered().title("paladin").border_style(border))
...
fn draw_help(f: &mut Frame, area: Rect) {
    let lines = vec![
        Line::from("paladin-tui key bindings"),
        Line::from(""),
