...
//! magic to choose between the native symcrypt path and the AES Crypt read path.
...
/// Native symcrypt magic (DESIGN §5.2).
const SYMCRYPT_MAGIC: &[u8] = b"SYMCRYPT";
...
pub(crate) enum Format {
    /// A symcrypt container (`header::parse` + `stream::*`).
    Symcrypt,
...
pub(crate) fn detect<R: Read>(mut input: R) -> Result<(Format, impl Read)> {
    let peek = read_up_to(&mut input, SYMCRYPT_MAGIC.len())?;

    let format = if peek.starts_with(AESCRYPT_MAGIC) {
        Format::AesCrypt
    } else if peek == SYMCRYPT_MAGIC {
        Format::Symcrypt
...
fn is_proper_prefix_of_magic(peek: &[u8]) -> bool {
    (peek.len() < SYMCRYPT_MAGIC.len() && SYMCRYPT_MAGIC.starts_with(peek))
...
mod tests {
    ...
    #[test]
    fn symcrypt_magic_is_detected_and_rechained() {
        let input = b"SYMCRYPT\x01the rest of the container";
        let (format, drained) = detect_and_drain(input).unwrap();
        assert_eq!(format, Format::Symcrypt);
    ...
    fn foreign_magic_is_bad_magic() {
        ...
        // Eight bytes that share a leading 'S' but are not SYMCRYPT.
        assert!(matches!(
            detect_and_drain(b"SYMCRYPXtail"),
            Err(SymError::BadMagic)
        ));
    }
    ...
    fn short_prefix_of_a_magic_is_malformed_not_bad() {
        // Proper prefixes of SYMCRYPT and of AES are treated as truncated headers.
        for short in [b"".as_slice(), b"S", b"SYM", b"SYMCRYP", b"A", b"AE"] {
            assert!(
