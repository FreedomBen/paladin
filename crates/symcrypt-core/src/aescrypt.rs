...
//!
//! This legacy format is **only ever read**; symcrypt never writes it. The
...
fn decrypt_impl<R: Read, W: Write>(
    ...
    // symcrypt-style keyfile component exists, and a password is required.
    if secret.has_keyfile() {
        return Err(SymError::InvalidOptions(
            "AES Crypt files do not use a symcrypt keyfile; pass the password directly (a UTF-8 --password-file may hold an AES Crypt key file)",
        ));
    }
...
/// Re-encode the password as UTF-16LE (PLAN_05 §2.4). The symcrypt front-ends
