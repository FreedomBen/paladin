...
const BEGIN_MARKER: &str = "-----BEGIN SYMCRYPT MESSAGE-----";
const END_MARKER: &str = "-----END SYMCRYPT MESSAGE-----";
            ...
            Phase::Header => {
                ...
                if trimmed.as_slice() == BEGIN_MARKER.as_bytes() {
                    ...
                    Err(SymError::MalformedHeader(
                        "expected the BEGIN SYMCRYPT MESSAGE marker",
                    ))
                }
            }
            Phase::Body => {
...
mod tests {
    ...
    fn auto_detect_distinguishes_binary_from_armored() {
        ...
        let binary = b"SYMCRYPT\x01rest-of-container";
